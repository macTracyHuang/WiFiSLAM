...
public:
    ...
    Sophus::SE3f TrackRGBD_Wifi(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const Fingerprint::FingerprintPtr& fingerprint, const vector<IMU::Point>& vImuMeas = vector<IMU::Point>(), string filename="");
    // Proccess the given monocular frame and optionally imu data
    // Input images: RGB (CV_8UC3) or grayscale (CV_8U). RGB is converted to grayscale.
    // Returns the camera pose (empty if tracking fails).
    Sophus::SE3f TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas = vector<IMU::Point>(), string filename="");
    Sophus::SE3f TrackMonocular_Wifi(const cv::Mat &im, const double &timestamp, const Fingerprint::FingerprintPtr& fingerprint , const vector<IMU::Point>& vImuMeas = vector<IMU::Point>(), string filename="");
    ...
    void SaveFrameInfo(const string &filename);
