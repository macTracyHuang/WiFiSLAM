 ...
 * 
 */
Sophus::SE3f System::TrackRGBD_Wifi(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const Fingerprint::FingerprintPtr& fingerprint, const vector<IMU::Point>& vImuMeas, string filename)
{
...
Sophus::SE3f System::TrackMonocular_Wifi(const cv::Mat &im, const double &timestamp, const Fingerprint::FingerprintPtr& fingerprint, const vector<IMU::Point>& vImuMeas, string filename)
{

    Verbose::PrintMess("TrackMonocular_Wifi", Verbose::VERBOSITY_DEBUG);
    {
        unique_lock<mutex> lock(mMutexReset);
        if(mbShutDown)
            return Sophus::SE3f();
    ...
    // 计算相机位姿
    Sophus::SE3f Tcw = mpTracker->GrabImageMonocular_Wifi(imToFeed,fingerprint,timestamp,filename);
