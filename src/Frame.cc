...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera,Frame* pPrevF, const IMU::Calib &ImuCalib)
    :mpcpi(NULL),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     ...
     mImuCalib(ImuCalib), mpImuPreintegrated(NULL), mpPrevFrame(pPrevF), mpImuPreintegratedFrame(NULL), mpReferenceKF(static_cast<KeyFrame*>(NULL)), mbIsSet(false), mbImuPreintegrated(false),
     mpCamera(pCamera),mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false),mbHasPoseWifi(false), minEigenValue(Eigen::VectorXd(6)), mHessian(Eigen::Matrix<double, 6, 6>::Zero()),minWiFiEigenValue(Eigen::VectorXd(6)),mWiFiHessian(Eigen::Matrix<double, 6, 6>::Zero())
{
    // Frame ID
    // Step 1 帧的ID 自增
    mnId=nNextId++;
...
//RGBD with wifi
// RGBD
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, const Fingerprint::FingerprintPtr& fingerprint, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera,Frame* pPrevF, const IMU::Calib &ImuCalib)
    :mpcpi(NULL),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     mTimeStamp(timeStamp), mK(K.clone()), mK_(Converter::toMatrix3f(K)),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth),
     mImuCalib(ImuCalib), mpImuPreintegrated(NULL), mpPrevFrame(pPrevF), mpImuPreintegratedFrame(NULL), mpReferenceKF(static_cast<KeyFrame*>(NULL)), mbIsSet(false), mbImuPreintegrated(false),
     mpCamera(pCamera),mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false), mpFingerprint(fingerprint),mbHasPoseWifi(false), minEigenValue(Eigen::VectorXd(6)), mHessian(Eigen::Matrix<double, 6, 6>::Zero()),minWiFiEigenValue(Eigen::VectorXd(6)),mWiFiHessian(Eigen::Matrix<double, 6, 6>::Zero())
{
    // Frame ID
    // Step 1 帧的ID 自增
    mnId=nNextId++;
...
bool Frame::HasWifi()
{
    // cout << "has wifi()" <<endl;
    if (mpFingerprint)
        return this->mpFingerprint->mvAp.size() > 0;
    else
        return false;
}
